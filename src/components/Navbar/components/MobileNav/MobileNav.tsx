import { mainNavLinks, mobileNavLinks, navLinks } from "@/constant";
...
const MobileNav: React.FC<MobileNavProps> = ({ menuOpen }) => {
      ...
      <div>
        <ul
          className={`p-0 max-h-0 h-auto absolute overflow-hidden transition-all duration-[.5s] ease-in-out bg-white left-0 right-0 max-w-full text-center m-0 ${
            menuOpen
              ? "min-h-[100vh] border-b-[1px] border-[#f1f1f1] shadow-[0_20px_20px_-2px_rgb(0_0_0_/_75%)] overflow-y-scroll"
              : null
          }`}
        >
          {mainNavLinks.map((link) => (
            <li
              key={link.id}
              className="border-b-2 border-[#eee]"
            >
              <span
                className={`leading-[55px] w-full text-[#333] text-[16px] font-bold block relative ${
